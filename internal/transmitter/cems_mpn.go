...
func (p *CemsMpnProtocol) cemsMpnLogin(station model.RelayStation) (*tokens.LoginToken, error) {
	...
	token := tokens.RegisterToken(station.Protocol, station.BaseURL, loginResp.LoginData.Token, "", loginResp.LoginData.TokenTTL - 5)
	return token, nil
}
...
func (p *CemsMpnProtocol) cemsMpnGetToken(task model.Transmission, station model.RelayStation) (*tokens.LoginToken, error) {
	...
	if token == nil {
fmt.Println("token is nil, logging in...")
		return p.cemsMpnLogin(station)
	}
	if token.IsExpired() {
fmt.Println("token is expired, logging in...")
		return p.cemsMpnLogin(station)
	}
